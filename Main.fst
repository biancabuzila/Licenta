...
let rec print_clause (clause : list int) : string
   = match clause with
      ...
      | hd::tl -> 
           ...
           else concat " \/ " [string_of_int hd; r]
...
let rec print_cnf_formula (f : list (list int)) : string
   = match f with
      ...
      | hd::tl ->
           ...
           else concat " /\ " [concat "" ["("; print_clause hd; ")"]; r]
...
let main = let f0 = Var 0 in
           ...
           let out2 = FStar.Printf.sprintf "%s\n" (pretty_print cnf) in
           print_string out2;

        //    let tseitin_f = tseitin test_formula in
           let tseitin_f = [[1;2;3]; [0;1]; [2;4]] in
           let out3 = FStar.Printf.sprintf "%s\n" (print_cnf_formula tseitin_f) in
           print_string out3
