...
let prove_same_value_not (f1:formula_t) (rf1 : list (list int)) (v1:int) 
                         (n:nat) (start_interval:nat) (v:int) (end_interval:nat)
                         (rf : list (list int)) 
    : Lemma (requires n <= start_interval /\ start_interval <= v /\
...
let prove_can_extend_not (f1:formula_t) (rf1 : list (list int)) (v1:int)
                         (n:nat) (start_interval:nat) (v:int) (end_interval:nat)
                         (rf : list (list int))
...
let prove_can_extend_and (f1:formula_t) (rf1 : list (list int)) (v1:int) 
                         (f2:formula_t) (rf2 : list (list int)) (v2:int) 
                         (n:nat) (start_interval:nat) (mid:nat) (v:int) (end_interval:nat)
                         (rf :list (list int))
    : Lemma (requires n <= start_interval /\ start_interval <= mid /\ mid <= v /\
...
let prove_can_extend_dimplies (f1:formula_t) (rf1 : list (list int)) (v1:int) 
                              (f2:formula_t) (rf2 : list (list int)) (v2:int) 
                              (n:nat) (start_interval:nat) (mid:nat) (v:int) (end_interval:nat)
                              (rf :list (list int))
    : Lemma (requires n <= start_interval /\ start_interval <= mid /\ mid <= v /\
    ...
    = LP.append_assoc rf1 rf2 (dimplies_clauses v1 v2 v);
      ...
      dimplies_clauses_in_interval v1 v2 v n start_interval mid end_interval;
      append_variables_in_interval rf1 rf2 n start_interval end_interval;
      append_variables_in_interval (rf1 @ rf2) (dimplies_clauses v1 v2 v) n start_interval end_interval;assert (valid (DImplies f1 f2) rf v n start_interval end_interval);
      
      let aux (tau : list bool) 
          : Lemma (requires L.length tau = n)
                  (ensures L.length tau = n /\
                           can_extend tau (DImplies f1 f2) rf v n start_interval end_interval)
          = let conditions1 (tau1 : list bool) : Tot bool =
                is_prefix tau tau1 &&
                L.length tau1 = mid &&
                truth_value_cnf_formula rf1 tau1 &&
            ...
            let tau' = combine tau tau1 tau2 n start_interval mid v (truth_value (DImplies f1 f2) tau) in
            assert (L.length tau' = v + 1);

            // assert (interval_of_list tau' 0 n = interval_of_list tau 0 n);
            assert (interval_of_list tau' start_interval mid = interval_of_list tau1 start_interval mid);
            assert (interval_of_list tau' mid v = interval_of_list tau2 mid v);
            assert (L.index tau' v = truth_value (DImplies f1 f2) tau);

            assert (truth_value_literal (pos_var_to_lit v) tau' = truth_value (DImplies f1 f2) tau);
            assert (truth_value_literal (neg_var_to_lit v) tau' = not (truth_value (DImplies f1 f2) tau));
            ...
            // assert (truth_value (DImplies f1 f2) tau = (truth_value f1 tau <==> truth_value f2 tau));

            lemma_dimplies_clauses v1 v2 v tau';
            assert (can_extend tau (DImplies f1 f2) rf v n start_interval end_interval)
      in
      forall_intro (move_requires aux)
#reset-options
