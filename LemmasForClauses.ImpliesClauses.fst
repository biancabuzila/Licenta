...
let prove_same_value_can_extend_implies (f1:formula_t) (rf1 : list (list int)) (v1:int)
    ...
    = LP.append_assoc rf1 rf2 (implies_clauses v1 v2 v);
      ...
      let aux (tau : list bool) 
          ...
          = assert (variables_up_to_cnf_formula rf (L.length tau));
            ...
            assert (variables_up_to_cnf_formula (implies_clauses v1 v2 v) (L.length tau));
           
            append_valid_cnf_formulas rf2 (implies_clauses v1 v2 v);
            append_variables_in_interval rf2 (implies_clauses v1 v2 v) n start_interval end_interval;
            assert (variables_in_interval (rf2 @ (implies_clauses v1 v2 v)) n start_interval end_interval);
            assert (variables_up_to_cnf_formula (rf2 @ (implies_clauses v1 v2 v)) (L.length tau));

            true_parts_of_cnf_formula (rf1 @ rf2) (implies_clauses v1 v2 v) tau;
            true_parts_of_cnf_formula rf1 (rf2 @ (implies_clauses v1 v2 v)) tau;
