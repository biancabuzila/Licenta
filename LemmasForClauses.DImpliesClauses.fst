module LemmasForClauses.DImpliesClauses
...
let lemma_dimplies_clauses (v1:int) (v2:int) (v:int) (tau : list bool)
    ...
    = let rf = dimplies_clauses v1 v2 v in
      ...
      assert (L.length rf = 4) by compute ();
...
let dimplies_clauses_in_interval (v1:int) (v2:int) (v:int) (n:nat) (start_interval:nat) (mid:nat) (end_interval:nat)
    ...
    = let rf = dimplies_clauses v1 v2 v in
      assert (forall clause . L.mem clause rf ==> variables_in_interval_clause clause n start_interval end_interval)


#set-options "--split_queries always"
let prove_same_value_can_extend_dimplies (f1:formula_t) (rf1 : list (list int)) (v1:int)
    ...
    : Lemma (requires n <= start_interval /\ start_interval <= mid /\ mid <= v /\
            ...
            (ensures valid (DImplies f1 f2) rf v n start_interval end_interval /\
                     ...
                     tseitin_can_extend (DImplies f1 f2) rf v n start_interval end_interval)
    = LP.append_assoc rf1 rf2 (dimplies_clauses v1 v2 v);
      ...
      let aux (tau : list bool) 
          ...
          = assert (variables_up_to_cnf_formula rf (L.length tau));
            ...
            assert (variables_up_to_cnf_formula (dimplies_clauses v1 v2 v) (L.length tau));
           
            append_valid_cnf_formulas rf2 (dimplies_clauses v1 v2 v);
            append_variables_in_interval rf2 (dimplies_clauses v1 v2 v) n start_interval end_interval;
            assert (variables_in_interval (rf2 @ (dimplies_clauses v1 v2 v)) n start_interval end_interval);
            assert (variables_up_to_cnf_formula (rf2 @ (dimplies_clauses v1 v2 v)) (L.length tau));

            true_parts_of_cnf_formula (rf1 @ rf2) (dimplies_clauses v1 v2 v) tau;
            true_parts_of_cnf_formula rf1 (rf2 @ (dimplies_clauses v1 v2 v)) tau;
            true_parts_of_cnf_formula rf1 rf2 tau;

            lemma_dimplies_clauses v1 v2 v tau
      in
      C.forall_intro (C.move_requires aux);
      assert (forall (tau : list bool) . L.length tau >= end_interval && truth_value_cnf_formula rf tau ==> (assert (variables_up_to (DImplies f1 f2) (L.length tau)); truth_value_literal (pos_var_to_lit v) tau = truth_value (DImplies f1 f2) tau))
#reset-options
